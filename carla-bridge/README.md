...
| 形态 | 路径 | 说明 |
|------|------|------|
| **Python** | `carla_bridge.py` | 完整 CARLA 相机 + ffmpeg 推流；`docker-compose` / 本地开发常用。 |
| **C++** | `carla-bridge/cpp/` | 轻量 MQTT + RTMP（无 LibCarLA 时为 testsrc 图案）；适合 CI/最小依赖验证。 |

两套均订阅 `vehicle/control`、发布 `vehicle/status`、向 ZLM 推 `{VIN}_cam_*` 四路流（见 [docs/TELEOP_SIGNAL_CONTRACT.md](../docs/TELEOP_SIGNAL_CONTRACT.md)）。**不要**在同一 Broker 上为同一 VIN 同时跑两套进程。
...
- **推流**：四路 RTMP 到 ZLM，app=`teleop`；流名为 `{VIN}_cam_front` 等（`VIN` 来自环境变量；空 VIN 时退化为无前缀 `cam_front`…），与 Teleop 客户端、`backend` WHEP 约定一致。
