...
HealthChecker::SystemReadiness HealthChecker::checkReadiness() const {
  ...
  // 详细信息
  readiness.details["overall_status"] =
      QVariant::fromValue(static_cast<int>(computeOverallStatus()));
  ...
  // 内存状态
  if (m_memoryTotalBytes > 0) {
    readiness.details["memory_usage_percent"] =
        100.0 * static_cast<double>(m_memoryUsedBytes) / m_memoryTotalBytes;
  }
...
void HealthChecker::updateComponentHealth(const QString& component, Status status,
  ...
  if (oldStatus != status) {
    ...
    qInfo() << "[HealthChecker] Component" << component
            << "status changed:" << static_cast<int>(oldStatus) << "->" << static_cast<int>(status)
            << "message:" << message;
  }
}
...
void HealthChecker::updateComponentTimeout(const QString& component, int timeoutMs) {
  ...
  if (it != m_components.end()) {
    ...
    map["timeoutEnabled"] = true;
    m_components[component] = map;
    qInfo() << "[HealthChecker] Created component" << component << "with timeout" << timeoutMs
            << "ms";
  }
}
...
void HealthChecker::updateVideoDecoderStatus(bool healthy, const QString& message) {
  ...
  updateComponentHealth("video_decoder", status,
                        message.isEmpty() ? (healthy ? "OK" : "Failed") : message);
}
...
void HealthChecker::updateMqttStatus(bool connected, const QString& message) {
  ...
  updateComponentHealth("mqtt", status,
                        message.isEmpty() ? (connected ? "Connected" : "Disconnected") : message);
}
...
void HealthChecker::updateControlLoopStatus(bool active, const QString& message) {
  ...
  updateComponentHealth("control_loop", status,
                        message.isEmpty() ? (active ? "Active" : "Inactive") : message);
}
...
void HealthChecker::updateGpuStatus(bool accelerated, const QString& message) {
  ...
  updateComponentHealth(
      "gpu", status,
      message.isEmpty() ? (accelerated ? "GPU accelerated" : "CPU fallback") : message);
}

void HealthChecker::updateNetworkQuality(bool ok, const QString& message) {
  Status status = ok ? Status::Healthy : Status::Degraded;
  updateComponentHealth("network", status,
                        message.isEmpty() ? (ok ? "Quality OK" : "Quality degraded") : message);
}
...
void HealthChecker::updateMemoryInfo(int64_t usedBytes, int64_t totalBytes) {
  ...
  qDebug() << "[HealthChecker] Memory updated: used=" << usedBytes << "total=" << totalBytes;
}
...
QVariantMap HealthChecker::getSystemMemoryUsage() {
  ...
  info["totalBytes"] =
      static_cast<qlonglong>(QSysInfo::machineHostName().size());  // 需要其他方式获取
...
void HealthChecker::updatePluginStatus(const QString& pluginName, bool loaded,
  ...
  qInfo() << "[HealthChecker] Plugin" << pluginName << "status: loaded=" << loaded
          << "version=" << version;
}
