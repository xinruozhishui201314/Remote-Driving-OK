                           ...
                           const QString& component)
{
    ...
    auto it = std::find_if(m_errors.begin(), m_errors.end(),
        [category, &message, &component](const Error& e) {
    ...
    if (it != m_errors.end()) {
        ...
        qDebug().noquote() << QString("[Client][ErrorRegistry] Aggregated error #%1: "
            "[%2][%3] %4 (occurrences: %5)")
            .arg(errorId)
    ...
    // 发射信号
    qWarning().noquote() << QString("[Client][ErrorRegistry] Error #%1: "
        "[%2][%3][%4] %5")
        .arg(errorId)
...
QVector<ErrorRegistry::Error> ErrorRegistry::getErrors(Category category,
                                                         int maxCount) const
{
    QMutexLocker lock(&m_mutex);
...
void ErrorRegistry::clearErrors(Category category)
{
    ...
    if (category == Category::Unknown) {
            ...
            std::remove_if(m_errors.begin(), m_errors.end(),
                [category, &removed, &fatalRemoved, &errorRemoved](const Error& e) {
                    if (e.category == category) {
                        if (e.level == Level::Fatal) fatalRemoved++;
                        if (e.level == Level::Error) errorRemoved++;
                        removed++;
                        return true;
                    }
                    return false;
        ...
        updateStats();
        qInfo() << "[Client][ErrorRegistry] Cleared errors for category" << catStr
...
QString ErrorRegistry::getErrorSummary() const
{
    ...
    for (int i = 0; i <= static_cast<int>(Category::System); ++i) {
        ...
        for (const auto& e : m_errors) {
            if (e.category == cat) {
                count++;
                switch (e.level) {
                case Level::Fatal: fatalCount++; break;
                case Level::Error: errorCount++; break;
    ...
    for (int i = 0; i < recentCount; ++i) {
        ...
        errObj["timestamp"] = QDateTime::fromMSecsSinceEpoch(e.timestampMs)
                                    .toString(Qt::ISODate);
...
void ErrorRegistry::updateStats()
{
    ...
    for (const auto& e : m_errors) {
        switch (e.level) {
        case Level::Fatal: fatal++; break;
        case Level::Error: error++; break;
        default: break;
        }
    }
...
QString ErrorRegistry::categoryToString(Category category)
{
    ...
    case Category::Video: return QStringLiteral("Video");
    case Category::Control: return QStringLiteral("Control");
    case Category::Auth: return QStringLiteral("Auth");
    case Category::Session: return QStringLiteral("Session");
    case Category::Safety: return QStringLiteral("Safety");
...
{
    if (str == QStringLiteral("Network")) return Category::Network;
    if (str == QStringLiteral("Video")) return Category::Video;
    if (str == QStringLiteral("Control")) return Category::Control;
    if (str == QStringLiteral("Auth")) return Category::Auth;
    if (str == QStringLiteral("Session")) return Category::Session;
    if (str == QStringLiteral("Safety")) return Category::Safety;
    if (str == QStringLiteral("System")) return Category::System;
    return Category::Unknown;
}

ErrorRegistry::Level ErrorRegistry::stringToLevel(const QString& str)
{
    if (str == QStringLiteral("Info")) return Level::Info;
    if (str == QStringLiteral("Warn")) return Level::Warn;
    if (str == QStringLiteral("Error")) return Level::Error;
    if (str == QStringLiteral("Fatal")) return Level::Fatal;
    return Level::Info;
}
