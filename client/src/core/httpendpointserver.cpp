  ...
  connect(m_server, &QTcpServer::newConnection, this, [this]() {
    while (m_server->hasPendingConnections()) {
      ...
      if (socket) {
        ...
        connect(
            socket, &QTcpSocket::readyRead, this, [this, socket]() { handleRequest(socket); },
            Qt::QueuedConnection);
...
HttpEndpointServer::~HttpEndpointServer() { stop(); }
...
bool HttpEndpointServer::start(int port) {
  ...
  if (!m_server->listen(QHostAddress::Any, static_cast<quint16>(port))) {
    ...
    if (!m_server->listen(QHostAddress::Any, 0)) {
      QString errorMsg =
          QString("Failed to listen on port %1: %2").arg(port).arg(m_server->errorString());
      qCritical() << "[Client][HTTP] " << errorMsg;
      emit serverError(errorMsg);
      return false;
    }
    qWarning() << "[Client][HTTP] Port" << port << "in use, falling back to"
               << m_server->serverPort();
  }

  // port==0 时也必须同步实际绑定端口（否则 m_port 仍为 0，探测与日志均异常）
...
void HttpEndpointServer::setHealthStatus(const QString& component, bool healthy,
...
QString HttpEndpointServer::buildHttpResponse(int statusCode, const QString& contentType,
                                              const QString& body) {
  QString statusText;
  switch (statusCode) {
    case 200:
...
QString HttpEndpointServer::handleMetrics() {
...

  {
    ...
    for (auto it = m_numericMetrics.constBegin(); it != m_numericMetrics.constEnd(); ++it) {
      output += QString("%1 %2 %3\n").arg(it.key()).arg(it.value(), 0, 'g', 15).arg(timestamp);
    }
  ...
  // 合并全进程 MetricsCollector 单例（EventBus / 视频 / main 连接计数等）
  output += MetricsCollector::instance().getPrometheusFormat();
...
QString HttpEndpointServer::handleMetricsJson() {
  ...
  root["metricsCollector"] =
      QJsonObject::fromVariantMap(MetricsCollector::instance().getAllMetrics());
