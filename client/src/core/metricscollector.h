 ...
 *
 * HTTP：HttpEndpointServer 的 /metrics 与 /metrics/json 会合并导出本单例（勿再为业务指标 new
 * 第二份）。
 */
class MetricsCollector : public QObject {
 ...
 public:
  ...
  // 设置指定指标的阈值
  Q_INVOKABLE void setThreshold(const QString& name, double warning, double critical,
  ...
  struct Metrics {
    ...
    static constexpr const char* SAFETY_LATENCY_VIOLATIONS_TOTAL =
        "safety_latency_violations_total";
