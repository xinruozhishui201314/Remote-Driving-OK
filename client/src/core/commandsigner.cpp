...
static const QStringList kSignedFields = {
    "steering", "throttle", "brake", "gear",
    "emergency_stop", "timestamp", "seq",
    "session_id", "vin"
};

// ─── setCredentials ──────────────────────────────────────────────────────────

void CommandSigner::setCredentials(const QString& vin,
...
{
    if (m_signingKey.isEmpty()) {
        if (reason) *reason = "no signing key";
        return false;
    }

    if (!json.contains("hmac")) {
        if (reason) *reason = "missing hmac field";
        qWarning() << "[Client][CommandSigner] received message without hmac";
        return false;
    }

    const QByteArray receivedMac =
        QByteArray::fromHex(json["hmac"].toString().toLatin1());
    ...
    if (receivedMac.size() != expected.size()) {
        if (reason) *reason = "hmac length mismatch";
        return false;
    }
    ...
    if (diff != 0) {
        if (reason) *reason = "hmac mismatch";
...
QByteArray CommandSigner::computeHmac(const QByteArray& payload) const
{
    return QMessageAuthenticationCode::hash(
        payload, m_signingKey, QCryptographicHash::Sha256);
}
...
QByteArray CommandSigner::canonicalize(const QJsonObject& json)
{
    ...
    for (const QString& field : kSignedFields) {
        if (!canonical.isEmpty()) canonical += '&';
        canonical += field + '=';
        if (json.contains(field)) {
            const QJsonValue v = json[field];
            if (v.isBool())   canonical += v.toBool() ? "true" : "false";
            else if (v.isDouble()) canonical += QString::number(v.toDouble(), 'f', 6);
            else canonical += v.toString();
        }
    }
...
QByteArray CommandSigner::deriveKey(const QString& vin,
                                     const QString& sessionId,
                                     const QString& token)
{
    ...
    return QMessageAuthenticationCode::hash(
        info, token.toUtf8(), QCryptographicHash::Sha256);
}
