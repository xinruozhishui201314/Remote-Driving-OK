...
void PerformanceMonitor::start(int reportIntervalMs)
{
    ...
    m_reportTimer.start();
    qInfo() << "[Client][PerformanceMonitor] started, report interval="
            << reportIntervalMs << "ms";
}
...
void PerformanceMonitor::recordVideoE2E(int64_t latencyUs)
{
    m_videoE2EStats.addSample(latencyUs);
}

void PerformanceMonitor::recordControlRTT(int64_t rttUs)
{
    m_controlRTTStats.addSample(rttUs);
}
...
void PerformanceMonitor::recordVideoFrame()
{
    m_videoFps.tick(TimeUtils::wallClockMs());
}

void PerformanceMonitor::recordControlTick()
{
    m_controlHz.tick(TimeUtils::wallClockMs());
}

void PerformanceMonitor::recordPacketLoss(double rate)
{
    m_packetLossRate = rate;
}

void PerformanceMonitor::recordDroppedFrame()
{
    ++m_droppedFrames;
}

void PerformanceMonitor::recordDecoderError()
{
    ++m_decoderErrors;
}
