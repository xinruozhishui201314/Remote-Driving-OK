...
void SystemStateMachine::setupTransitions()
{
    ...
    // ── DRIVING ──────────────────────────────────────────────────────────────
    addTransition(S::DRIVING, T::STOP_SESSION, S::STOPPING, nullptr, [this]() {
        qInfo().noquote() << "[Client][FSM] DRIVING -> STOPPING";
    });
...
bool SystemStateMachine::fire(Trigger trigger)
{
    ...
    SystemState newS{};
    {
        ...
        if (it == m_transitions.end()) {
            qWarning().noquote() << "[Client][FSM] invalid transition state="
                                 << stateToString(m_current)
                                 << " trigger=" << triggerToString(trigger);
            return false;
        }
        tr = it->second;
        if (tr.guard && !tr.guard()) {
            qDebug().noquote() << "[Client][FSM] guard rejected trigger="
    ...
    qInfo().noquote() << "[Client][FSM] transition" << oldName << "->" << newName
                      << "via" << triggerToString(trigger);
