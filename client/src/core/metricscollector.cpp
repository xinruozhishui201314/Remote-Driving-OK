...
QVariantMap MetricsCollector::getAllMetrics() const {
  ...
  // 添加系统指标
  result["_system_uptime_ms"] =
      static_cast<qlonglong>(QDateTime::currentMSecsSinceEpoch() - m_startTimeMs);
...
QString MetricsCollector::getPrometheusFormat() const {
  ...
  for (auto it = m_metrics.begin(); it != m_metrics.end(); ++it) {
    ...
    // 转换指标名称为 Prometheus 合法名：驼峰→下划线，点号→下划线（与 rule_files 告警一致）
    ...
    promName.replace(QLatin1Char('.'), QLatin1Char('_'));

    switch (mv.type) {
      case MetricType::Counter:
        stream << "# TYPE " << promName << " counter\n";
        {
          // 指标名已以 _total 结尾时不再追加，避免出现 client.foo_total_total
          const QString lineKey = promName.endsWith(QLatin1String("_total"), Qt::CaseInsensitive)
                                      ? promName
                                      : (promName + QLatin1String("_total"));
          stream << lineKey << " " << QString::number(mv.value, 'f', 2) << "\n";
        }
        break;
      case MetricType::Gauge:
      ...
      case MetricType::Histogram:
        ...
        stream << promName << "_avg "
               << QString::number(mv.count > 0 ? mv.sum / static_cast<double>(mv.count) : 0.0, 'f',
                                  2)
               << "\n";
...
void MetricsCollector::setThreshold(const QString& name, double warning, double critical,
                                    bool lowerIsWorse) {
  ...
  qInfo() << "[MetricsCollector] Threshold set for" << name << "warning=" << warning
          << "critical=" << critical << "lowerIsWorse=" << lowerIsWorse;
...
void MetricsCollector::checkThresholds(const QString& name, double value) {
  ...
  if (exceeded) {
    ...
    qWarning() << "[MetricsCollector] Metric exceeded critical threshold:" << name << "=" << value
               << "threshold=" << t.critical;
  } else if (isWarning) {
    // 单独发射 warning 级别信号
    emit metricThresholdWarning(name, value, thresholdValue);
    qInfo() << "[MetricsCollector] Metric warning threshold:" << name << "=" << value
