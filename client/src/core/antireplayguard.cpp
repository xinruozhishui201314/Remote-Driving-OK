...
bool AntiReplayGuard::checkAndRecordInternal(uint32_t seq, int64_t timestampMs,
                                              int64_t localTimeMs, QString* reason)
{
...
void AntiReplayGuard::handleOverflow(uint32_t seq)
{
    ...
    qWarning() << "[Client][AntiReplay] OVERFLOW detected! counter=" << m_overflowCount.load(std::memory_order_relaxed)
                << "lastSeq=" << m_highestSeq << "newSeq=" << seq
...
QString AntiReplayGuard::lastResetReason() const
{
