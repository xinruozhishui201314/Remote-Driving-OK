...
QString Tracing::generateSpanId()
{
    ...
    spanId.reserve(16);
    spanId = QString("%1%2")
        .arg(rand1, 8, 16, QChar('0'))
        .arg(rand2, 8, 16, QChar('0'));
...
Tracing::Span Tracing::beginSpan(const QString& name, const QString& operation)
{
    ...
    // 记录活跃追踪数
    METRICS_OBSERVE("tracing_spans_active", static_cast<double>(m_totalSpansStarted.load() - m_totalSpansEnded.load()));

    qDebug().noquote() << "[Tracing] Span started: traceId=" << traceId
...
double Tracing::samplingRate() const
{
    return m_samplingRate.load(std::memory_order_relaxed);
}
