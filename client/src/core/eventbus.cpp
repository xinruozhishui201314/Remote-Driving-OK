                                           ...
                                           std::function<void(const ClientEvent&)> handler)
{
    ...
    MetricsCollector::instance().set("eventbus_subscribers_total", static_cast<double>(totalSubscribers));
                        ...
                        std::shared_ptr<ClientEvent> event)
{
...

    {
        ...
        while (m_queue.size() >= MAX_QUEUE_DEPTH) {
            // 弹出最低优先级的最老事件
            if (!m_queue.empty()) {
                auto cmp = [](const QueuedEvent& a, const QueuedEvent& b) {
                    return a > b;
                };
                ...
                if (dropped % 100 == 0 || dropped <= 3) {
                    qWarning().noquote() << "[Client][EventBus] Queue overflow, dropped lowest priority event. total_dropped=" << dropped;
                }
        ...
        if (currentDepth > m_lastQueueMaxDepth.load()) {
            ...
            MetricsCollector::instance().set("eventbus_queue_max_depth", static_cast<double>(currentDepth));
        }
        ...
        if (currentDepth >= MAX_QUEUE_DEPTH * 8 / 10) {
            ...
            if (nowMs - s_lastOverflowWarningMs < 1000) {
                ...
                if (s_overflowWarningCount >= 4) return; // 1秒内最多4次警告
            }
            ...
            qWarning().noquote() << "[Client][EventBus] Queue fill ratio high: " << currentDepth << "/" << MAX_QUEUE_DEPTH;
        }
    }

    // 更新队列深度指标（带锁外更新以避免锁竞争）
    MetricsCollector::instance().set("eventbus_queue_depth", static_cast<double>(currentDepth));
    MetricsCollector::instance().set("eventbus_queue_fill_ratio", static_cast<double>(currentDepth) / static_cast<double>(MAX_QUEUE_DEPTH));
...
void EventBus::processPending()
{
    ...
    std::vector<QueuedEvent> local;
    {
        ...
        MetricsCollector::instance().set("eventbus_queue_fill_ratio", static_cast<double>(m_queue.size()) / static_cast<double>(MAX_QUEUE_DEPTH));
    }
    ...
    while (!local.empty()) {
        ...
        if (dispatchLatencyMs >= DISPATCH_LATENCY_WARNING_MS) {
            qWarning().noquote() << "[Client][EventBus] High dispatch latency: " << dispatchLatencyMs << "ms for typeId=" << qe.event->typeId();
    ...
    if (dispatchCount > 0) {
        MetricsCollector::instance().increment("eventbus_dispatch_total", static_cast<double>(dispatchCount));
    }

    // 记录处理耗时到 MetricsCollector
    int64_t processingTimeNs = currentTimeNs() - processingStartNs;
    MetricsCollector::instance().observe("eventbus_dispatch_latency_ms", static_cast<double>(processingTimeNs / 1'000'000));
}
...
int EventBus::subscribe(int typeId, std::function<void(const LegacyEvent&)> handler)
{
    ...
    // 实际的 Legacy 事件通过 eventDispatched 信号传递
    {
        ...
        m_subscribers[legacyKey].push_back(Subscriber{h, [](const ClientEvent&) {
            // 空操作；Legacy 订阅者通过 eventDispatched 信号接收事件
        }});
    }
    ...
    qDebug() << "[Client][EventBus] Legacy subscribe: typeId=" << typeId
             << "handle=" << h << "(signal-based, not wrapper-based)";
    return h;
}
...
EventBus::QueueStats EventBus::getQueueStats() const
{
    ...
    // 获取分发统计
    {
        ...
        if (m_totalDispatched.load(std::memory_order_relaxed) > 0) {
            stats.avgDispatchLatencyMs = static_cast<double>(m_totalDispatchLatencyNs) /
