...
protected:
    ...
    double clamp(double value, double min, double max,
                 const char* name, bool doClamp = true) {
        if (std::isnan(value)) {
            reportError(ErrorRegistry::Category::System,
                QString("NaN detected for %1").arg(name),
                ErrorRegistry::Level::Warn, metaObject()->className());
            return 0.0;
        }
        if (std::isinf(value)) {
            reportError(ErrorRegistry::Category::System,
                QString("Inf detected for %1").arg(name),
                ErrorRegistry::Level::Warn, metaObject()->className());
            return 0.0;
        }
        if (value < min || value > max) {
            if (doClamp) {
                ...
                qWarning().nospace() << "[" << metaObject()->className() << "] "
                    << name << " clamped: " << value << " -> " << clamped;
                return clamped;
            } else {
                reportError(ErrorRegistry::Category::System,
                    QString("%1 out of range: %2 (valid: [%3, %4])")
    ...
    float clamp(float value, float min, float max,
                const char* name, bool doClamp = true) {
        return static_cast<float>(clamp(static_cast<double>(value),
    ...
    int clampInt(int value, int min, int max,
        ...
        if (value < min || value > max) {
            if (doClamp) {
                ...
                qWarning().nospace() << "[" << metaObject()->className() << "] "
                    << name << " (int) clamped: " << value << " -> " << clamped;
                return clamped;
            } else {
                reportError(ErrorRegistry::Category::System,
    ...
    unsigned int clampUInt(unsigned int value, unsigned int min, unsigned int max,
                           const char* name, bool doClamp = true) {
        if (value < min || value > max) {
            if (doClamp) {
                const unsigned int clamped = qBound(min, value, max);
                qWarning().nospace() << "[" << metaObject()->className() << "] "
                    << name << " (uint) clamped: " << value << " -> " << clamped;
                return clamped;
            } else {
                reportError(ErrorRegistry::Category::System,
    ...
    qint64 clampInt64(qint64 value, qint64 min, qint64 max,
                       const char* name, bool doClamp = true) {
        if (value < min || value > max) {
            if (doClamp) {
                const qint64 clamped = qBound(min, value, max);
                qWarning().nospace() << "[" << metaObject()->className() << "] "
                    << name << " (int64) clamped: " << value << " -> " << clamped;
                return clamped;
            } else {
                reportError(ErrorRegistry::Category::System,
    ...
    void reportError(ErrorRegistry::Category category,
                      const QString& message,
                      ErrorRegistry::Level level,
