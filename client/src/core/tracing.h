...
public:
    ...
    struct Span {
        ...
        int64_t durationMs() const {
            if (endTimeNs == 0 || startTimeNs == 0) return 0;
...
#define METRICS_INC(...) \
    METRICS_INC_IMPL(__VA_ARGS__, 1.0)
...
#define TRACE_SCOPE(name) \
    auto __tracing_span = Tracing::instance().beginSpan(name)

#define TRACE_OPERATION(name, op) \
    auto __tracing_span = Tracing::instance().beginSpan(name, op)

#define TRACE_END() \
    Tracing::instance().endSpan(__tracing_span)

// 日志宏（自动添加 traceId）
#define TRACE_LOG_CONTEXT() \
    Tracing::instance().addTraceToContext(QVariantMap())
