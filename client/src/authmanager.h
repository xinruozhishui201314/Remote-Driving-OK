...
class AuthManager : public QObject {
 ...
 public slots:
  ...
  /** 单元测试专用：不经网络登录即可注入后端基址与访问令牌（SessionManager 等） */
 ...
 private:
  void updateLoginStatus(bool loggedIn, const QString &username = QString(),
                         const QString &token = QString());
  void saveCredentials();
  void loadCredentials();

  // JWT 安全存储
  void storeTokensSecurely(const QString &accessToken, const QString &refreshToken,
                           qint64 expiresIn);
