...
void VehicleStatus::updateStatus(const QJsonObject &status)
{
    ...
    if (status.contains("speed")) {
        ...
        if (qAbs(m_speed - newSpeed) > 0.01) {
            setSpeed(newSpeed);
            hasChanges = true;
            if (shouldLog) changedFields << QString("speed:%1").arg(newSpeed, 0, 'f', 1);
        }
    }
    if (status.contains("battery")) {
        ...
        if (qAbs(m_batteryLevel - newBattery) > 0.1) {
            ...
            hasChanges = true;
            if (shouldLog) changedFields << QString("battery:%1").arg(newBattery, 0, 'f', 1);
        }
    }
    if (status.contains("gear")) {
        ...
        if (g.isString()) {
            ...
            if (newGear.isEmpty()) newGear = "N";
        } else         if (g.isDouble()) {
            int gi = static_cast<int>(g.toDouble());
            if (gi == -1) newGear = "R";
            else if (gi == 0) newGear = "N";
            else if (gi == 1) newGear = "D";
            else if (gi == 2) newGear = "P";
            else newGear = "N";
        }
        if (m_gear != newGear) {
            ...
            setGear(newGear);
            hasChanges = true;
            if (shouldLog) changedFields << QString("gear:%1").arg(newGear);
        }
    }
    if (status.contains("steering")) {
        ...
        if (qAbs(m_steering - newSteering) > 0.001) {
            ...
            hasChanges = true;
            if (shouldLog) changedFields << QString("steering:%1").arg(newSteering, 0, 'f', 3);
        }
    }
    if (status.contains("odometer")) {
        ...
        if (qAbs(m_odometer - newOdometer) > 0.01) {
            ...
            hasChanges = true;
            if (shouldLog) changedFields << QString("odometer:%1").arg(newOdometer, 0, 'f', 2);
        }
    }
    if (status.contains("voltage")) {
        ...
        if (qAbs(m_voltage - newVoltage) > 0.1) {
            ...
            hasChanges = true;
            if (shouldLog) changedFields << QString("voltage:%1").arg(newVoltage, 0, 'f', 1);
        }
    }
    if (status.contains("network_rtt")) {
        ...
        if (qAbs(m_networkRtt - newRtt) > 1.0) {
            ...
            hasChanges = true;
            if (shouldLog) changedFields << QString("rtt:%1ms").arg(newRtt, 0, 'f', 0);
        }
    }
    if (status.contains("current")) {
        ...
        if (qAbs(m_current - newCurrent) > 0.1) {
            ...
            hasChanges = true;
            if (shouldLog) changedFields << QString("current:%1").arg(newCurrent, 0, 'f', 1);
        }
    }
    if (status.contains("temperature")) {
        ...
        if (qAbs(m_temperature - newTemp) > 0.1) {
            ...
            hasChanges = true;
            if (shouldLog) changedFields << QString("temperature:%1").arg(newTemp, 0, 'f', 1);
        }
    }
    ...
    if (status.contains("remote_control_enabled")) {
        ...
        if (m_remoteControlEnabled != newRemoteControlEnabled) {
            qDebug() << "[CLIENT][VEHICLE_STATUS] remote_control_enabled:" << m_remoteControlEnabled << "->" << newRemoteControlEnabled;
            setRemoteControlEnabled(newRemoteControlEnabled);
            hasChanges = true;
            if (shouldLog || isAckMessage) changedFields << QString("rc_enabled:%1").arg(newRemoteControlEnabled ? "1" : "0");
        }
    }
    if (status.contains("driving_mode")) {
        QString newDrivingMode = status["driving_mode"].toString().trimmed();
        if (newDrivingMode.isEmpty()) newDrivingMode = "自驾";
        if (m_drivingMode != newDrivingMode) {
            qDebug() << "[CLIENT][VEHICLE_STATUS] driving_mode:" << m_drivingMode << "->" << newDrivingMode;
            setDrivingMode(newDrivingMode);
            hasChanges = true;
            if (shouldLog || isAckMessage) changedFields << QString("mode:%1").arg(newDrivingMode);
        }
    }
    ...
    if (status.contains("sweep_active")) {
        ...
        if (m_sweepActive != newSweepActive) {
            ...
            hasChanges = true;
            if (shouldLog) changedFields << QString("sweep:%1").arg(newSweepActive ? "1" : "0");
        }
    }
    ...
    if (status.contains("brake")) {
        ...
        if (m_brakeActive != newBrakeActive) {
            ...
            hasChanges = true;
            if (shouldLog) changedFields << QString("brake:%1").arg(newBrakeActive ? "1" : "0");
        }
    }

    // ★ 水箱/垃圾箱/清扫进度（环卫车遥测）
    if (status.contains("water_tank_level") || status.contains("waterTankLevel")) {
        double v = status.contains("water_tank_level") ? status["water_tank_level"].toDouble(75.0) : status["waterTankLevel"].toDouble(75.0);
        setWaterTankLevel(v);
        if (shouldLog) changedFields << QString("water_tank:%1").arg(v, 0, 'f', 1);
    }
    if (status.contains("trash_bin_level") || status.contains("trashBinLevel")) {
        double v = status.contains("trash_bin_level") ? status["trash_bin_level"].toDouble(40.0) : status["trashBinLevel"].toDouble(40.0);
        setTrashBinLevel(v);
        if (shouldLog) changedFields << QString("trash_bin:%1").arg(v, 0, 'f', 1);
    }
    if (status.contains("cleaning_current") || status.contains("cleaningCurrent")) {
        int v = status.contains("cleaning_current") ? status["cleaning_current"].toInt(400) : status["cleaningCurrent"].toInt(400);
        setCleaningCurrent(v);
        if (shouldLog) changedFields << QString("cleaning_current:%1").arg(v);
    }
    if (status.contains("cleaning_total") || status.contains("cleaningTotal")) {
        int v = status.contains("cleaning_total") ? status["cleaning_total"].toInt(500) : status["cleaningTotal"].toInt(500);
        setCleaningTotal(v);
        if (shouldLog) changedFields << QString("cleaning_total:%1").arg(v);
    }

    if (isAckMessage) {
        qDebug() << "[CLIENT][VEHICLE_STATUS] ack processed: rc=" << m_remoteControlEnabled << "mode=" << m_drivingMode;
    }
