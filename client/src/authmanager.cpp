...
void AuthManager::login(const QString &username, const QString &password,
  ...
  qDebug().noquote() << "[Client][Auth] 发起登录: username=" << username
                     << " serverUrl=" << serverUrl;
  if (m_currentReply) {
  ...
  Q_UNUSED(username);
  Q_UNUSED(password);  // 占位，避免下方 if 消失导致引用未定义
#else
  ...
  if (username == QLatin1String("123") && password == QLatin1String("123")) {
    ...
    QString testToken =
        QStringLiteral("test_token_") + QString::number(QDateTime::currentMSecsSinceEpoch());
  ...
  connect(m_currentReply, &QNetworkReply::finished, this,
          [this]() { onLoginReply(m_currentReply); });
  connect(m_currentReply, &QNetworkReply::errorOccurred, this,
          [this](QNetworkReply::NetworkError error) {
            QNetworkReply *reply = m_currentReply;
            QString errStr = reply ? reply->errorString() : QString();
            qDebug().noquote() << "[Client][Auth] errorOccurred: code=" << static_cast<int>(error)
                               << " errorString=" << errStr << " url="
                               << (reply && reply->url().isValid() ? reply->url().toString()
                                                                   : QString());
...
void AuthManager::onLoginReply(QNetworkReply *reply) {
  ...
  if (!reply) {
    // Qt 会先发 errorOccurred 再发 finished；errorOccurred 里已把 m_currentReply 置空并 emit
    // 了真实错误，此处不再重复 emit 避免覆盖为「无响应」
    qDebug().noquote()
        << "[Client][Auth] onLoginReply: reply=null（已由 errorOccurred 处理，不再重复 emit）";
    return;
  }
  int statusCode = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
  if (reply->error() != QNetworkReply::NoError) {
    QString err = reply->errorString();
    qDebug().noquote() << "[Client][Auth] onLoginReply: HTTP 错误 statusCode=" << statusCode
                       << " error=" << err << " → loginFailed";
  ...
  reply->deleteLater();
  qDebug().noquote() << "[Client][Auth] onLoginReply: HTTP" << statusCode
                     << " bodySize=" << data.size();

  QJsonParseError parseErr;
  QJsonDocument doc = QJsonDocument::fromJson(data, &parseErr);
  if (parseErr.error != QJsonParseError::NoError) {
    qDebug().noquote() << "[Client][Auth] onLoginReply: 响应非 JSON offset=" << parseErr.offset
                       << " snippet=" << QString::fromUtf8(data.left(150));
  ...
  if (json.contains("error")) {
    QString desc = json.contains("error_description") ? json["error_description"].toString()
                                                      : json["error"].toString();
    qDebug().noquote() << "[Client][Auth] onLoginReply: Keycloak error=" << json["error"].toString()
                       << " description=" << desc << " → loginFailed";
  ...
  if (token.isEmpty()) {
    qDebug().noquote() << "[Client][Auth] onLoginReply: 响应中无 access_token/token keys="
                       << json.keys().join(QLatin1Char(',')) << " → loginFailed(响应中无令牌)";
  ...
  qDebug().noquote() << "[Client][Auth] 收到令牌: expiresIn=" << expiresIn
                     << "s, hasRefreshToken=" << !refreshToken.isEmpty();
  ...
  if (!jwtPayload.isEmpty()) {
    ...
    if (jwtPayload.contains("exp")) {
      ...
      qDebug().noquote() << "[Client][Auth] JWT exp=" << m_tokenExpiresIn
                         << " iat=" << m_tokenIssuedAt;
    }
  }
  ...
  qDebug().noquote() << "[Client][Auth] onLoginReply: 登录成功 username=" << username
                     << " tokenLen=" << token.size() << " → emit loginSucceeded";
  emit loginSucceeded(token, userInfo);
}

void AuthManager::logout() {
  QString prevUser = m_username;
  qDebug().noquote() << "[Client][Auth] 退出登录: username="
                     << (prevUser.isEmpty() ? "(空)" : prevUser);
...
void AuthManager::refreshToken() {
  ...
  connect(m_refreshReply, &QNetworkReply::finished, this,
          [this]() { onRefreshReply(m_refreshReply); });
...
void AuthManager::forceRefreshToken() {
  ...
  if (!m_isLoggedIn || m_refreshToken.isEmpty()) {
    qWarning().noquote()
        << "[Client][Auth] forceRefreshToken: 无法强制刷新（未登录或无 refresh token）";
    logout();
    return;
  }
...
void AuthManager::onTokenExpiryCheck() {
  ...
  if (isTokenExpiringSoon(300) && !m_refreshToken.isEmpty()) {
    qDebug().noquote() << "[Client][Auth] 令牌即将过期（剩余" << (m_tokenExpiresIn - now)
                       << "秒），自动刷新";
    forceRefreshToken();
  }
}
...
void AuthManager::addUsernameToHistory(const QString &username) {
  ...
  QSettings settings;
  settings.setValue("auth/usernameHistory", m_usernameHistory);
  qDebug().noquote() << "[Client][Auth] 保存账户名历史 count=" << m_usernameHistory.size()
                     << " 新增=" << u;
...
void AuthManager::loadCredentials() {
  ...
  qDebug().noquote() << "[Client][Auth] loadCredentials: username="
                     << (m_username.isEmpty() ? "(空)" : m_username)
...
void AuthManager::setUnitTestServerUrl(const QString &url) {
  m_serverUrl = url;
  emit serverUrlChanged(m_serverUrl);
}

void AuthManager::setUnitTestAuthToken(const QString &token) {
  m_authToken = token;
  emit authTokenChanged(m_authToken);
}
...
void AuthManager::storeTokensSecurely(const QString &accessToken, const QString &refreshToken,
                                      qint64 expiresIn) {
  if (!m_keystore) {
    qWarning() << "[AuthManager] KeystoreManager 不可用，使用 QSettings fallback";
    QSettings settings;
    settings.setValue("auth/token", accessToken);
    settings.setValue("auth/tokenExpiry",
                      expiresIn > 0 ? QDateTime::currentSecsSinceEpoch() + expiresIn : 0);
    return;
  }
