...
// 登录页仍可能为 localhost（宿主机视角）；容器内 localhost 不可达 Backend。此时用环境变量中的服务地址探测。
...
{
    const QString envB = QProcessEnvironment::systemEnvironment().value(QStringLiteral("BACKEND_URL"));
    if (envB.isEmpty())
        return serverUrl;
...
{
    const QString envK = QProcessEnvironment::systemEnvironment().value(QStringLiteral("KEYCLOAK_URL"));
    if (!envK.isEmpty()) {
        const QUrl su(serverUrl);
        if (isLocalLoopbackHost(su.host())) {
            QUrl kc(envK);
...
void NodeHealthChecker::refresh(const QString &serverUrl)
{
    ...
    if (m_backendReply) { m_backendReply->abort(); m_backendReply->deleteLater(); m_backendReply = nullptr; }
    if (m_keycloakReply) { m_keycloakReply->abort(); m_keycloakReply->deleteLater(); m_keycloakReply = nullptr; }
    if (m_zlmReply) { m_zlmReply->abort(); m_zlmReply->deleteLater(); m_zlmReply = nullptr; }
    ...
    if (!backendHealthUrl.path().endsWith(QLatin1String("health"))) {
        ...
        backendHealthUrl.setPath((p.isEmpty() || p == QLatin1String("/")) ? QLatin1String("/health") : (p + (p.endsWith(QLatin1Char('/')) ? QLatin1String("") : QLatin1String("/")) + QLatin1String("health")));
    }
    m_pendingCount++;
    ...
    if (zlmBase.isEmpty()) {
        ...
        if (!zlmBase.endsWith(QLatin1Char('/'))) zlmBase += QLatin1Char('/');
...
void NodeHealthChecker::onBackendFinished()
{
    ...
    if (reply->error() != QNetworkReply::NoError) {
        qDebug().noquote() << "[Client][节点检测] Backend 请求失败:" << reply->errorString() << "url=" << reply->url().toString();
...
void NodeHealthChecker::onKeycloakFinished()
{
    ...
    int code = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
    if (reply->error() != QNetworkReply::NoError) {
        qDebug().noquote() << "[Client][节点检测] Keycloak 请求失败:" << reply->errorString() << "url=" << reply->url().toString();
...
void NodeHealthChecker::onZlmFinished()
{
    ...
    int code = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
    if (reply->error() != QNetworkReply::NoError) {
        qDebug().noquote() << "[Client][节点检测] ZLM 请求失败:" << reply->errorString() << "url=" << reply->url().toString();
...
void NodeHealthChecker::tryFinish()
{
    ...
    if (m_pendingCount <= 0) {
        setChecking(false);
        qDebug().noquote() << "[Client][节点检测] 全部完成 Backend=" << m_backendStatus << " Keycloak=" << m_keycloakStatus << " ZLM=" << m_zlmStatus;
        emit checkFinished();
    }
}
