...
bool deepDiagEnabled()
{
    static const bool on =
        QProcessEnvironment::systemEnvironment().value(QStringLiteral("CLIENT_X11_DEEP_DIAG")) ==
        QStringLiteral("1");
    return on;
}
...
void logXcbRequestBudget(const char *tag)
{
    ...
    if (!app || app->platformName() != QLatin1String("xcb")) {
        qInfo().noquote() << "[Client][X11Diag][" << tag << "] skip xcb budget (no app or platform!=xcb)";
        return;
    }
...
void hookQuickWindow(QQuickWindow *qw)
{
    ...
    QObject::connect(qw, &QQuickWindow::frameSwapped, qw,
                     [qw, frameTag]() {
                         ...
                         qInfo().nospace().noquote()
                             << "[Client][X11Diag][Frame] frameSwapped n=" << n << " win=\"" << qw->title()
                             << "\" size=" << qw->width() << "x" << qw->height()
                             << " edpr=" << qw->effectiveDevicePixelRatio();
                     },
                     Qt::DirectConnection);
}
...
void mergePreAppLoggingRules(QString &inOutFilterRules)
{
    ...
    fprintf(stderr,
            "[Client][X11Diag] CLIENT_X11_DEEP_DIAG=1: enabling qt.qpa.xcb.debug (see Qt QLoggingCategory)\n");
}
...
void installAfterQmlLoaded(QObject *appParentForTimers)
{
    ...
    qInfo().noquote() << "[Client][X11Diag] CLIENT_X11_DEEP_DIAG=1 已启用 — 周期性采样 X 单包上限与窗口像素缓冲粗算";
