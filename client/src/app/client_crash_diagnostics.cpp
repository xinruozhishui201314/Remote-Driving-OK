...
void logExtraEnvForX11Gpu(const QProcessEnvironment &env)
{
    ...
    qCritical().noquote()
        << "[Client][CrashDiag][xcb][REQ_LEN_EXCEED] 渲染进度快照: QQuickWindow::sceneGraphInitialized 已触发次数="
        ...
        << " (若为 0 表示断连发生在首次 swap/present 前后)";
    {
        ...
        if (!g_glRendererUtf8.isEmpty() || !g_glVendorUtf8.isEmpty()) {
            qCritical().noquote()
                ...
                << QString::fromUtf8(g_glVendorUtf8) << " GL_RENDERER="
                << QString::fromUtf8(g_glRendererUtf8) << " GL_VERSION="
                << QString::fromUtf8(g_glVersionUtf8);
        } else {
            qCritical().noquote()
                << "[Client][CrashDiag][xcb][REQ_LEN_EXCEED] 尚无 GL 字符串缓存（sceneGraphInitialized 未跑完或未创建 GL 上下文）";
        }
    }
}
...
#if defined(CLIENT_HAVE_XCB)
// 启动时 xcb_get_maximum_request_length（四字节单位，见 libxcb 文档）；REQ_LEN_EXCEED 时连接已坏，只能复用该快照做定量对照
 ...
 * 单条 X 请求若超过该上限，服务器可关闭连接并导致 xcb_connection_has_error()==XCB_CONN_CLOSED_REQ_LEN_EXCEED。
...
void logReqLenExceedFramebufferHypothesis()
{
    ...
    if (units == 0) {
        qCritical().noquote()
            << "[Client][CrashDiag][xcb][REQ_LEN_EXCEED][size] boot maximum_request_length 未记录（连接在启动阶段已异常？）"
               " — 跳过整帧 vs 单包上限对照";
        return;
    }
    const quint64 ceilingBytes = quint64(units) * 4u;
    qCritical().nospace().noquote()
        << "[Client][CrashDiag][xcb][REQ_LEN_EXCEED][size] boot xcb_get_maximum_request_length=" << units
        << " four-byte units (libxcb) => single-request payload ceiling ~ " << ceilingBytes
...
{
    constexpr char kMsg[] = "[Client][CrashDiag] SIGABRT received (often follows assert/qFatal/abort)\n";
...
public:
    bool nativeEventFilter(const QByteArray &eventType, void *message, qintptr *result) override
    {
        ...
        qCritical().nospace().noquote()
            ...
            << " (see X.org XErrorEvent / xcb_generic_error_t)";
        return false;
    }
};
...
QString annotateIfX11Broke(const QString &msg)
{
    static const QRegularExpression re(
        QStringLiteral(R"(The X11 connection broke \(error (\d+)\))"),
...
void installAfterQGuiApplication(QGuiApplication &app)
{
    ...
    if (app.platformName() == QLatin1String("xcb")) {
        ...
        qInfo().noquote() << "[Client][CrashDiag] xcb QAbstractNativeEventFilter 已安装（将记录 X protocol error 事件）";

        // libxcb: xcb_get_maximum_request_length — 服务器允许的单条请求长度上限（字节级线索，用于对照 REQ_LEN_EXCEED）
        // 文档: https://xcb.freedesktop.org/manual/group__XCB__Core__API.html
        if (auto *x11Boot = app.nativeInterface<QNativeInterface::QX11Application>()) {
            if (xcb_connection_t *bootConn = x11Boot->connection()) {
                if (xcb_connection_has_error(bootConn) == 0) {
                    ...
                    const QByteArray disp = QProcessEnvironment::systemEnvironment()
                                                .value(QStringLiteral("DISPLAY"))
                                                .toUtf8();
                    const quint64 approxCeilingBytes = quint64(maxReq) * 4u;
                    qInfo().nospace().noquote()
                        << "[Client][CrashDiag][xcb] boot DISPLAY=" << disp
                        << " xcb_get_maximum_request_length=" << maxReq
                        << " four-byte units (libxcb doc) ~ " << approxCeilingBytes
        ...
        QObject::connect(poll, &QTimer::timeout, &app, []() {
            ...
            if (err != 0 && err != s_lastLogged) {
                ...
                qCritical().nospace().noquote()
                    ...
                    << " (libxcb: 连接已不可用；Qt 将 qWarning 并 exit，见 qtbase qxcbconnection.cpp processXcbEvents)";
                ...
                if (err == 4) {
                    ...
                    if (g_elapsedStarted.load(std::memory_order_relaxed)) {
                        qCritical().nospace().noquote()
                            ...
                            << g_sinceGuiAppReady.elapsed()
                            << " (用于对照首帧/布局稳定与 xcb 断连时刻)";
                    }
                    qCritical().noquote()
                        << "[Client][CrashDiag][xcb][REQ_LEN_EXCEED] 现场 DISPLAY="
                    ...
                    qCritical().noquote()
                        << "[Client][CrashDiag][xcb][REQ_LEN_EXCEED] 下一步：主机侧 x11trace/xtrace 抓包，或 "
                           "QT_LOGGING_RULES=\"qt.qpa.xcb.debug=true\"（见 Qt QLoggingCategory 文档）定位过大请求来源；"
                           "或 CLIENT_X11_DEEP_DIAG=1 启用 frameSwapped/GL 字符串采样（见 client_x11_deep_diag.h）";
                }
            }
        });
        poll->start(250);
        ...
        qInfo().noquote() << "[Client][CrashDiag] 非 xcb 平台 name=" << app.platformName() << " 跳过 xcb 过滤器/轮询";
    }
#else
    Q_UNUSED(app);
...
void installAfterTopLevelWindowsReady()
{
    ...
    for (QWindow *w : windows) {
        ...
        // 渲染线程：对照 libxcb REQ_LEN_EXCEED 与「整窗 RGBA 单包」假说（Khronos: glGet GL_MAX_TEXTURE_SIZE 等）
        QObject::connect(
            ...
            [qw]() {
                ...
                if (ctx) {
                    ...
                    fmtStr = QStringLiteral("rgba=%1/%2/%3/%4 depth=%5 stencil=%6 samples=%7 swapInterval=%8")
                    ...
                    if (QOpenGLFunctions *f = ctx->functions()) {
                        const char *vendor = reinterpret_cast<const char *>(f->glGetString(0x1F00)); // GL_VENDOR
                        const char *renderer = reinterpret_cast<const char *>(f->glGetString(0x1F01)); // GL_RENDERER
                        const char *version = reinterpret_cast<const char *>(f->glGetString(0x1F02)); // GL_VERSION
                        const QMutexLocker locker(&g_glMetaMutex);
    ...
    if (hooked == 0) {
        qWarning().noquote() << "[Client][CrashDiag] 未发现 QQuickWindow，sceneGraphError 未连接（检查 QML 根是否为 Window）";
    } else {
        qInfo().noquote() << "[Client][CrashDiag] QQuickWindow::sceneGraphError 已连接 count=" << hooked;
    }
}
