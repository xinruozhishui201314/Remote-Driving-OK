...
void ErrorRecoveryManager::setConfig(const RecoveryConfig& cfg) { m_config = cfg; }
...
void ErrorRecoveryManager::reportError(ErrorCategory category, const QString& description) {
  ...
  qWarning() << "[Client][ErrorRecovery] error reported"
             << "category=" << static_cast<int>(category) << "desc=" << description
...
void ErrorRecoveryManager::clearError(ErrorCategory category) {
  ...
  qInfo() << "[Client][ErrorRecovery] cleared error category=" << static_cast<int>(category);
}
...
void ErrorRecoveryManager::attemptRecovery() {
  ...
  qInfo() << "[Client][ErrorRecovery] attempting recovery level=" << static_cast<int>(level)
  ...
  if (success) {
    qInfo() << "[Client][ErrorRecovery] recovery succeeded level=" << static_cast<int>(level);
...
void ErrorRecoveryManager::escalate(ErrorRecord& record) {
  ...
  switch (record.currentLevel) {
    case RecoveryLevel::AUTO_RETRY:
      record.currentLevel = RecoveryLevel::SERVICE_RESTART;
      break;
    case RecoveryLevel::SERVICE_RESTART:
      record.currentLevel = RecoveryLevel::SESSION_REBUILD;
      break;
    case RecoveryLevel::SESSION_REBUILD:
      record.currentLevel = RecoveryLevel::SAFE_STOP;
      break;
    case RecoveryLevel::SAFE_STOP:
      break;
  }
  record.retryCount = 0;
  qWarning() << "[Client][ErrorRecovery] escalated from level" << static_cast<int>(old) << "to"
             << static_cast<int>(record.currentLevel);
}

int ErrorRecoveryManager::calculateRetryDelayMs(const ErrorRecord& record) const {
  const double delay = m_config.baseRetryMs * std::pow(m_config.backoffFactor, record.retryCount);
