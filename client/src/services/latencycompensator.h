...
class LatencyCompensator {
 public:
  ...
  /** maxPredictionAngleRad：对归一化转向的外推结果相对当前值的夹紧半宽（与 steeringAngle [-1,1]
   * 同量纲）。 */
  explicit LatencyCompensator(double processingDelayMs = 5.0, double maxPredictionAngleRad = 0.15);

  PredictionResult predict(const IInputDevice::InputState& current, double currentRTTMs);
