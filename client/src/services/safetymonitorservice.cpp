...
SafetyMonitorService::SafetyMonitorService(VehicleStatus* vs,
    ...
    , m_fsm(fsm)
{
    ...
    connect(&m_safetyTimer, &QTimer::timeout,
...
void SafetyMonitorService::setConfig(const Config& cfg)
{
    m_config = cfg;
}
...
bool SafetyMonitorService::initialize()
{
    ...
    qInfo() << "[Client][SafetyMonitorService] initialized"
            << "checkHz=" << kSafetyCheckHz
            << "deadman=" << m_config.deadmanTimeoutMs << "ms"
...
{
    m_lastHeartbeatMs  = TimeUtils::nowMs();
    m_missedHeartbeats = 0;
}

void SafetyMonitorService::onOperatorActivity()
{
    m_lastOperatorActivityMs = TimeUtils::nowMs();
...
void SafetyMonitorService::checkHeartbeat()
{
    ...
    if (elapsed > m_config.heartbeatTimeoutMs) {
        ...
        const QString msg = QString("[Client][SafetyMonitorService] heartbeat missed #%1 elapsed=%2ms")
                                .arg(m_missedHeartbeats).arg(elapsed);

        if (m_missedHeartbeats >= m_config.missedBeforeEmergency) {
            qCritical().noquote() << msg;
            triggerEmergencyStop(QString("Heartbeat lost: %1 consecutive misses").arg(m_missedHeartbeats));
            m_missedHeartbeats = 0;
...
void SafetyMonitorService::checkOperatorActivity()
{
    ...
    if (m_deadmanActive.load() && inactiveMs > m_config.inactivityTimeoutMs) {
        const QString msg = QString("[Client][SafetyMonitorService] operator inactive for %1ms")
                                .arg(inactiveMs);
        qWarning().noquote() << msg;
        emit safetyWarning(msg);
...
void SafetyMonitorService::checkDeadman()
{
    ...
    if (m_deadmanActive.load() && inactiveMs > m_config.deadmanTimeoutMs) {
        qCritical() << "[Client][SafetyMonitorService] DEADMAN timeout"
                    << inactiveMs << "ms without activity";
