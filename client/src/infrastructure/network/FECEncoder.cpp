...
FECEncoder::FECEncoder(double redundancy)
...
std::vector<QByteArray> FECEncoder::encode(const std::vector<QByteArray>& packets)
{
    ...
    size_t maxLen = 0;
    for (const auto& p : packets) maxLen = std::max(maxLen, static_cast<size_t>(p.size()));
        ...
        for (int j = 0; j < n; ++j) {
            if (j % r == i) {
                ...
                for (int k = 0; k < static_cast<int>(maxLen); ++k) {
                    repair[k] = static_cast<char>(
                        static_cast<uint8_t>(repair[k]) ^
                                             ...
                                             int originalCount)
{
    ...
    for (int g = 0; g < r; ++g) {
        ...
        if (missingIdx == -1 || !canRecover) continue;

        // Recover via XOR
        const int repairIndex = originalCount + g;
        if (repairIndex >= static_cast<int>(received.size())) continue;

        size_t maxLen = 0;
        for (const auto& p : result) maxLen = std::max(maxLen, static_cast<size_t>(p.size()));
        ...
        for (int j = g; j < originalCount; j += r) {
            ...
            if (!(receivedMask & (1u << j))) continue;
            const auto& src = result[j];
            for (int k = 0; k < static_cast<int>(maxLen); ++k) {
                recovered[k] = static_cast<char>(
                    static_cast<uint8_t>(recovered[k]) ^
                    static_cast<uint8_t>(k < src.size() ? src[k] : 0));
            }
        }
