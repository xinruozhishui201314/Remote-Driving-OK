...
BitrateDecision AdaptiveBitrateController::evaluate(const NetworkMetrics& metrics)
{
    ...
    if (smoothed != m_currentBitrateKbps) {
        qDebug() << "[Client][ABR] bitrate change" << m_currentBitrateKbps
                 << "->" << smoothed << "kbps"
                 << "delay_grad=" << delayGradient
...
double AdaptiveBitrateController::calculateDelayGradient(const NetworkMetrics& metrics)
{
    ...
    if (m_delayHistory.size() < kMinSamplesForGradient) return 0.0;
...
{
    SignalState state;
    if (gradient > 0.01)       state = SignalState::Overuse;
    else if (gradient < -0.01) state = SignalState::Underuse;
    else                        state = SignalState::Normal;
...
BitrateDecision AdaptiveBitrateController::makeBitrateDecision(
    uint32_t bitrate, const NetworkMetrics& metrics)
{
    BitrateDecision d;
