  ...
  connect(
      ...
      [this]() {
        qInfo() << "[Client][ControlLoopTicker] thread finished"
                << "totalTicks=" << m_index.load() << "missed=" << m_tickMissedCount.load()
...
void ControlLoopTicker::tickerThread() {
  ...
  while (!m_stopRequested.load()) {
    ...
    const auto actualIntervalNs =
        std::chrono::duration_cast<std::chrono::nanoseconds>(actualInterval).count();
    ...
    if (actualIntervalNs > expectedIntervalNs * 1.5) {
      ...
      qWarning().noquote() << "[Client][ControlLoopTicker] ★ TICK MISSED ★"
                           << "actualMs=" << nsToMs(actualIntervalNs) << "expectedMs=" << interval
    ...
    if (nextWakeTime < now2) {
      ...
      if (driftMs > interval) {
        qWarning().noquote() << "[Client][ControlLoopTicker] ★ TIMING DRIFT DETECTED ★"
                             << "driftMs=" << driftMs << "resetting to current time";
