...
{
    if (m_mqtt) {
        connect(m_mqtt, &MqttController::connectionStatusChanged,
                this, [this](bool connected) {
                    if (connected) onMqttConnected();
                    else onMqttDisconnected();
                });
        // Route vehicle status messages to TELEMETRY receivers
        connect(m_mqtt, &MqttController::statusReceived,
...
void MqttTransportAdapter::shutdown()
{
...
{
    if (m_mqtt) m_mqtt->disconnectFromBroker();
    m_state = ConnectionState::DISCONNECTED;
    emit connectionStateChanged(m_state);
    emit disconnected();
}

SendResult MqttTransportAdapter::send(TransportChannel channel,
                                       const uint8_t* data, size_t len,
                                       SendFlags /*flags*/)
{
    if (!m_mqtt) return SendResult{false, "No MQTT controller"};
...
NetworkQuality MqttTransportAdapter::getNetworkQuality() const
{
    return m_quality;
}
...
ConnectionState MqttTransportAdapter::connectionState() const
{
    return m_state;
}
...
void MqttTransportAdapter::setSessionCredentials(const QString& vin,
...
void MqttTransportAdapter::onMqttMessageReceived(const QString& topic,
                                                   const QByteArray& payload)
{
    // 主题路由
    TransportChannel ch = TransportChannel::TELEMETRY; // default
    if (topic.contains("control")) ch = TransportChannel::CONTROL_CRITICAL;
    else if (topic.contains("signal")) ch = TransportChannel::SIGNALING;

    const int64_t localNow = TimeUtils::wallClockMs();

    // ── 对遥测/状态消息做反重放验证 ──────────────────────────────────
    if (m_signingEnabled && (ch == TransportChannel::TELEMETRY ||
                              ch == TransportChannel::CONTROL_CRITICAL))
    {
        ...
        if (!doc.isNull() && doc.isObject()) {
            ...
            const uint32_t seq = static_cast<uint32_t>(
                obj.value("seq").toDouble(0));
            const int64_t msgTs = static_cast<int64_t>(
                obj.value("timestamp").toDouble(0));
