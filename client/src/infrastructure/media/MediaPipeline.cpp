...
bool MediaPipeline::initialize(const PipelineConfig& config)
{
    ...
    m_framePool = std::make_shared<FramePool>(
        config.framePoolSize, config.maxWidth, config.maxHeight, config.gpuMemoryType);
...
void MediaPipeline::decodeLoop()
{
    ...
    while (m_running) {
        ...
        // 提交到解码器
        auto res = m_decoder->submitPacket(
            reinterpret_cast<const uint8_t*>(nalu.data.constData()),
            static_cast<size_t>(nalu.data.size()),
            nalu.pts, nalu.pts);
        ...
        while (m_decoder->receiveFrame(rawFrame) == DecodeResult::Ok) {
            ...
            poolFrame->captureTimestamp = rawFrame.captureTimestamp > 0
                ? rawFrame.captureTimestamp : TimeUtils::wallClockMs();
                ...
                for (int p = 0; p < 3; ++p) {
                    if (rawFrame.planes[p].data && poolFrame->planes[p].data) {
                        const size_t sz = std::min(
                            static_cast<size_t>(rawFrame.planes[p].size),
