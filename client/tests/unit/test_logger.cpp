...
void TestLogger::test_droppedCount()
{
    ...
    QVERIFY2(dropped > 0 || dropped == 0, 
             "Dropped count should be >= 0 (implementation dependent)");
...
void TestLogger::test_rotationConfig()
{
    ...
    QVERIFY2(retrievedConfig.filePrefix == "test-rotation",
             qPrintable(QString("Expected prefix 'test-rotation', got '%1'").arg(retrievedConfig.filePrefix)));
}
