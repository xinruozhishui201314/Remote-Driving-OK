...
void TestThreadPool::test_submitWorkerPriority()
{
    ...
    // 提交 Worker 优先级任务
    pool.submit(ThreadPool::Priority::Worker, [&executed]() {
...
void TestThreadPool::test_submitHighPriority()
{
    ...
    // 提交 High 优先级任务
    pool.submit(ThreadPool::Priority::High, [&executed]() {
        executed.fetch_add(1, std::memory_order_relaxed);
    });
...
void TestThreadPool::test_submitTimeCriticalPriority()
{
    ...
    pool.submit(ThreadPool::Priority::TimeCritical, [&executed]() {
        executed.fetch_add(1, std::memory_order_relaxed);
    });
...
void TestThreadPool::test_waitForDone()
{
    ...
    // 验证所有任务完成
    QVERIFY2(counter.load() == taskCount, 
             qPrintable(QString("Expected %1 tasks completed, got %2").arg(taskCount).arg(counter.load())));
}
