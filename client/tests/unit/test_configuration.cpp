...
void TestConfiguration::test_loadFromFile()
{
    ...
    QVERIFY2(rateHz == 100,
             qPrintable(QString("Expected 100, got %1").arg(rateHz)));
...
void TestConfiguration::test_set()
{
    ...
    QVERIFY2(value == "test_value", qPrintable(QString("Expected 'test_value', got '%1'").arg(value)));
    ...
    QVERIFY2(nested == "nested_value", qPrintable(QString("Expected 'nested_value', got '%1'").arg(nested)));
}
...
void TestConfiguration::test_convenienceMethods()
{
    ...
    config.set("media.hardware_decode", false);
    ...
    QVERIFY2(config.enableHardwareDecode() == false,
              qPrintable(QString("enableHardwareDecode() mismatch: %1").arg(config.enableHardwareDecode())));
}
