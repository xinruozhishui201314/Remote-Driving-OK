...
### Shell（mosquitto_pub 规范示例）

仓库根目录：

```bash
source scripts/lib/mqtt_control_json.sh
mosquitto_pub -h 127.0.0.1 -p 1883 -t vehicle/control -m "$(mqtt_json_start_stream carla-sim-001)"
```

`mqtt_json_*` 函数会写入 `schemaVersion`（默认 `1.2.0`）、`timestampMs` 与 `seq`（与 `timestampMs` 同毫秒的整数，便于在 `$(...)` 子 shell 中仍唯一）。`vehicle/status` 模拟样本见 `mqtt_json_vehicle_status_chassis_sample`。
...
- **控制面** `vehicle/control`：时间戳字段为 **`timestampMs`**（毫秒整数），见 `mqtt/schemas/vehicle_control.json`。
- **状态面** `vehicle/status`：时间戳字段为 **`timestamp`**（毫秒整数），见 `mqtt/schemas/vehicle_status.json`。
