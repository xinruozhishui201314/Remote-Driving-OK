...
### 4.1 API 契约验证（`scripts/validate_api.py`）
...
| 阶段 | 目标 | 交付物 |
|------|------|--------|
| **MVP（当前 – 1 周）** | OpenAPI 契约、MQTT Schema 定义、WHIP/WHEP 统一算法 | `backend/api/openapi.yaml`、`mqtt/schemas/*.json`、Backend 统一 URL 函数 |
| **V1（2 周）** | 验证工具、运行时集成、CI/CD | `scripts/validate_api.py`、`scripts/validate_mqtt_schemas.py`、CI 加入契约验证 |
| **V2（1 个月）** | 代码生成、SDK、完整覆盖率 | `sdk/client/`（C++/Python）、SDK 文档、各模块改用 SDK |
